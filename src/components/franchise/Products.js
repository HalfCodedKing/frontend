...
const Products = () => {
  ...
  const fetchSelectedProductIds = async () => {
    try {
      const response = await axios.get('https://air2ca.onrender.com/franchise/products');
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        const productDetailsPromises = selectedProductIds.map(async (id) => {
          const response = await axios.get(`https://air2ca.onrender.com/franchise/products/${id}`);
          if (response.status === 200) {
            return {
