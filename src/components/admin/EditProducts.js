...
import { useNavigate, BrowserRouter as Router, useParams } from "react-router-dom";
...
export default function EditProducts() {
    ...
    const fetchProductDetails = async () => {
      try {
        const response = await axios.get(
          `http://https://air2ca.onrender.com/admin/products/${id}`
        );

        if (response.status === 200) {
          console.log('the products data is :-', response.data.products);
          const productsData = response.data.products;
          if (productsData.tabs) {
            console.log('tabs are :-', JSON.parse(productsData.tabs));
            productsData.tabs = JSON.parse(productsData.tabs);
          }
          if (productsData.variantCombinations) {
            productsData.productName = productsData.productUrl
            productsData.variantCombinations = JSON.parse(productsData.variantCombinations)
          }
          if (productsData.productName) {
            setPName(productsData.productName)
          }

          console.log('the products again data is :-', response.data.products);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://https://air2ca.onrender.com/admin/updateproducts/${id}`,
        formData,
        {
          headers: {
  ...
  const handleProductNameChange = (e) => {
    ...
    const newProductUrl = newProductName;
    // Use the product name as the URL
    setProductInfo((prevProductInfo) => ({
      ...prevProductInfo,
  ...
  const columns = productInfo.variantCombinations.map((section) => ({
    title: section.name,
    dataIndex: section.name,
    key: section.name,
  }));
  console.log('columns are :-' ,productInfo.variantCombinations)
  ...
  return (
    <body>
        ...
        <form onSubmit={handleSubmit} enctype="multipart/form-data">
          ...
          <div>
            ...
            <input
              ...
              multiple
            />
            <span onClick={handleAddMoreImages} className="btn-10" style={{ padding: '8px', textAlign: 'center', borderRadius: '13px', cursor: 'pointer', margin: '13px', width: '150px' }}>
          ...
          {
            console.log('tabs got', tabs)
          }

          {productInfo.tabs.map((tab, index) => (
            <div key={index}>
              {console.log('tab is ', productInfo.tabs)}
          ...
          <Space direction="vertical" style={{ width: "100%" }}>
            {variantSections.map((section, sectionIndex) => (
              <div key={sectionIndex}>
                {console.log('var combs are :-', productInfo.variantCombinations)}
                <Space direction="vertical" style={{ width: "100%" }}>
      ...
      <div className="left-form">
        ...
        <p>
          1. Finally, click the "Update Product" button at the end of the form to
          submit your product.
        </p>
      </div>
    </body>
