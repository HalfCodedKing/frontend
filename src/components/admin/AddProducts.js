...
export default function AddProducts() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://air2ca.onrender.com/admin/add-product",
        formData,
        {
          headers: {
  ...
  const handleVarChange = (value, sectionIndex, optionIndex) => {
    ...
    // Update productInfo with the new variants
    const variants = updatedSections.map((section) => ({
      name: section.name,
  ...
  const handleGenerateCombinations = () => {
    ...
    setProductInfo((prevProductInfo) => ({
      ...
      variantCombinations: newData.map((item) => ({
        ...item,
        excelData: item.uploadExcel,
      })),
      variants: variantSections.map((section) => ({
        name: section.name,
        options: section.options,
      })),
    }));
