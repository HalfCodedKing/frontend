...
export default function Products() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get('https://air2ca.onrender.com/admin/products');
  ...
  const handleDeleteClick = async () => {
    if (deleteProductId) {
      try {
        ...
        const response = await axios.delete(`https://air2ca.onrender.com/admin/products/${deleteProductId}`);
